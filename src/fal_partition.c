 ...
 * When this option is disable, it will auto find the partition table on a specified location in
 * flash partition.
 */
...
USED static const struct fal_partition partition_table_def[] SECTION("FalPartTable") =
    FAL_PART_TABLE;
...
void fal_show_part_table(void)
{
    ...
    log_i("| %-*.*s | %-*.*s |   offset   |    length  |", part_name_max, FAL_DEV_NAME_MAX, item1,
    ...
    for (i = 0; i < partition_table_len; i++)
    {
        ...
        log_i("| %-*.*s | %-*.*s | 0x%08lx | 0x%08x |", part_name_max, FAL_DEV_NAME_MAX, part->name,
              flash_dev_name_max, FAL_DEV_NAME_MAX, part->flash_name, part->offset, part->len);
    }
    log_i("=============================================================");
}
...
int fal_partition_init(void)
{
    ...
    while (part_table_find_ok)
    {
        ...
        partition_table =
            (fal_partition_t)FAL_REALLOC(partition_table, table_item_size * (table_num + 1));
    ...
    if (table_num == 0)
    {
        log_e("Partition table NOT found on flash: %s (len: %d) from offset: 0x%08x.",
...
int fal_partition_read(const struct fal_partition *part, uint32_t addr, uint8_t *buf, size_t size)
{
    ...
    ret = flash_dev->ops.read(flash_dev->ops.ops_arg, part->offset + addr, buf, size);
    if (ret < 0)
    {
 ...
 *           -1: error
 */
int fal_partition_write(const struct fal_partition *part, uint32_t addr, const uint8_t *buf,
                        size_t size)
{
    int                         ret       = 0;
    const struct fal_flash_dev *flash_dev = NULL;
    ...
    ret = flash_dev->ops.write(flash_dev->ops.ops_arg, part->offset + addr, buf, size);
    if (ret < 0)
    {
...
int fal_partition_erase(const struct fal_partition *part, uint32_t addr, size_t size)
{
    ...
    ret = flash_dev->ops.erase(flash_dev->ops.ops_arg, part->offset + addr, size);
    if (ret < 0)
    {
